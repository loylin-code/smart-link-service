...
> 📘 **架构设计文档**: 详细的架构设计请参考 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) 和 [docs/ARCHITECTURE_DIAGRAMS.md](docs/ARCHITECTURE_DIAGRAMS.md)

### 企业级架构目标

SmartLink 正在演进为企业级多租户 Agent 应用管理平台，目标支持：

- **1,000+** 并发 WebSocket 连接
- **多租户**架构（共享数据库 + tenant_id 隔离）
- **OAuth2 + JWT** 认证体系
- **分布式 Agent 池**与任务队列
- **水平扩展**能力
